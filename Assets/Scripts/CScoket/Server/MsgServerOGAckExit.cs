...
{
	private CmdRegisterFactory<MsgServerOGAckExit> CmdRegisterFactoryCmdServerv = new CmdRegisterFactory<MsgServerOGAckExit>(CmdDefine.OGID_REQ | CmdDefine.GATE_TAKE_ACHIEVEMENT);
}
public class MsgServerOGAckExit : CmdServer {

	private const string NOTIFY_MSG = "服务器 --> 客户端 关闭按钮返回消息:";
	
	private OGAckExit recvMsg;
	
	#region 构造函数
	
	public MsgServerOGAckExit()
		: base("MsgServerOGAckExit", CmdDefine.OGID_REQ | CmdDefine.GATE_TAKE_ACHIEVEMENT)
	{
	...
	{
		recvMsg = UtilNet.DeserializeNetMsg<OGAckExit>(msg);
		if (null == recvMsg)
		{
	...
	protected override void LogicDo()
	{
		...
		NetworkAccessFacade.Instance.DoFreeCoinsRsp(recvMsg);         
	}
