...
{
	private CmdRegisterFactory<MsgServersTOP> CmdRegisterFactoryCmdServerstop = new CmdRegisterFactory<MsgServersTOP>(CmdDefine.OGID_REQ | CmdDefine.GATE_BUY_MEMBERSTOP);
}
public class MsgServersTOP : CmdServer {

	private const string NOTIFY_MSG = "服务器 --> 客户端 关闭按钮返回消息:";
	
	private OGAckStopGame recvMsg;
	
	#region 构造函数
	
	public MsgServersTOP()
		: base("MsgServersTOP", CmdDefine.OGID_REQ | CmdDefine.GATE_BUY_MEMBERSTOP)
	{
	...
	{
		recvMsg = UtilNet.DeserializeNetMsg<OGAckStopGame>(msg);
		if (null == recvMsg)
		{
	...
	protected override void LogicDo()
	{
		...
		NetworkAccessFacade.Instance.Dostop(recvMsg);         
	}
