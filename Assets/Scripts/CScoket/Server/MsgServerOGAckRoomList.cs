...
{
	private CmdRegisterFactory<MsgServerOGAckRoomList> CmdRegisterFactoryCmdServerRoomList = new CmdRegisterFactory<MsgServerOGAckRoomList>(CmdDefine.OGID_REQ | CmdDefine.GATE_KICK_OUT);
}
public class MsgServerOGAckRoomList : CmdServer {

	private const string NOTIFY_MSG = "服务器 --> 客户端 房间列表:";
	
	private OGAckRoomList recvMsg;
	
	#region 构造函数
	
	public MsgServerOGAckRoomList()
		: base("MsgServerOGAckRoomList", CmdDefine.OGID_REQ | CmdDefine.GATE_KICK_OUT)
	{
	...
	{
		recvMsg = UtilNet.DeserializeNetMsg<OGAckRoomList>(msg);
		if (null == recvMsg)
		{
	...
	protected override void LogicDo()
	{
		...
		NetworkAccessFacade.Instance.DoLevelRoomRsp(recvMsg);         
	}
