...
public class ClientMsgOGReqDoubleGetGold : CmdClientBase {
	
	private const string SEND_MSG = "客户端 --> 服务器 比倍收分请求:";
	private const string RECV_MSG = "服务器 --> 客户端 比倍收分响应:";
	private const UInt32 Client = (UInt32)12345+(UInt32)52;
	
	/// <summary>
	/// 玩家准备
	/// </summary>
	private OGReqDoubleGetGold Login1;
	
	#region 构造函数
	
	public ClientMsgOGReqDoubleGetGold(OGReqDoubleGetGold log1)
	...
	protected override void DoSendMsg()
	{
		...
		byte[] sendByte = UtilNet.SerializeNetMsg<OGReqDoubleGetGold>(Login1);
		if (null == sendByte)
		{
	...
	{
		OGReqDoubleGetGold recvMsg = UtilNet.DeserializeNetMsg<OGReqDoubleGetGold>(msg);
		if (null == recvMsg)
		{
		...
		NetworkAccessFacade.Instance.SendTrustPlayReq(recvMsg);
	}
