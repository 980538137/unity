...
public class ClientMsgOGReqStartGame : CmdClientBase {
	...
	/// 玩家准备
	/// </summary>
	private OGReqStartGame Login1;
	private UInt32 clent=(UInt32)12345+(UInt32)50;
	
	#region 构造函数
	
	public ClientMsgOGReqStartGame(OGReqStartGame log1)
	...
	protected override void DoSendMsg()
	{
		...
		byte[] sendByte = UtilNet.SerializeNetMsg<OGReqStartGame>(Login1);
		if (null == sendByte)
		{
		...
		SendClientMsg.Instance.SendMsg(sendByte, sendByte.Length,clent , Cseq);
	...
	{
		OGReqStartGame recvMsg = UtilNet.DeserializeNetMsg<OGReqStartGame>(msg);
		if (null == recvMsg)
		{
		...
		NetworkAccessFacade.Instance.SendFreeCoinsReq(recvMsg);
	}
