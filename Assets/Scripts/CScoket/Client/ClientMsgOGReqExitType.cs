...
public class ClientMsgOGReqExitType : CmdClientBase {
	...
	/// 玩家准备
	/// </summary>
	private OGReqExitType Login1;
	private UInt32 Client=(UInt32)12345+(UInt32)4;
	#region 构造函数
	
	public ClientMsgOGReqExitType(OGReqExitType log1)
	...
	protected override void DoSendMsg()
	{
		...
		byte[] sendByte = UtilNet.SerializeNetMsg<OGReqExitType>(Login1);
		if (null == sendByte)
		{
	...
	{
		OGReqExitType recvMsg = UtilNet.DeserializeNetMsg<OGReqExitType>(msg);
		if (null == recvMsg)
		{
		...
		NetworkAccessFacade.Instance.SendLevelRoomReq(recvMsg);
	}
